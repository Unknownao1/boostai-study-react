(function () {
  var mount = document.getElementById("boostai-entry-workspace");
  if (!mount) return;

  var subjects = [
    { slug: "mathematics", name: "Mathematics", icon: "📐", description: "Algebra, ratios, graphs, and exam-style numerical reasoning." },
    { slug: "physics", name: "Physics", icon: "⚡", description: "Motion, energy, circuits, and clear step-by-step calculations." },
    { slug: "chemistry", name: "Chemistry", icon: "⚗️", description: "Moles, formulas, bonding, and reaction calculations." },
    { slug: "biology", name: "Biology", icon: "🧬", description: "Processes, data interpretation, and simple science explanations." },
    { slug: "english", name: "English", icon: "📝", description: "Readable literature analysis and clear writing prompts." },
    { slug: "history", name: "History", icon: "🏛️", description: "Cause, consequence, interpretation, and evidence-based answers." },
    { slug: "geography", name: "Geography", icon: "🌍", description: "Fieldwork, data analysis, ecosystems, and human geography." }
  ];
  ...
  var state = {
    subject: localStorage.getItem("boostai-main-subject") || "mathematics",
    questions: {}
  };
  ...
  function setSubject(subjectSlug) {
    ...
    localStorage.setItem("boostai-main-subject", subjectSlug);
    state.questions[subjectSlug] = buildQuestionsForSubject(subjectSlug);
    render();
  }

  function regenerateQuestion(questionId) {
    state.questions[state.subject] = (state.questions[state.subject] || []).map(function (question) {
      if (question.id !== questionId) return question;
      var blueprint = (blueprintMap[state.subject] || []).find(function (item) {
        return item.id === questionId;
      });
      return blueprint ? buildQuestion(state.subject, blueprint) : question;
    });
    render();
    var status = mount.querySelector("#boostai-main-status");
    if (status) {
  ...
  function toggleSolution(questionId) {
    (state.questions[state.subject] || []).forEach(function (question) {
  ...
  function render() {
    ...
    mount.innerHTML =
      '<section class="boostai-main-workspace">' +
        '<div class="boostai-main-workspace-head">' +
          '<div>' +
            '<h2 id="boostai-main-title">' + escapeHtml(subject.name + " study workspace") + "</h2>" +
            '<p id="boostai-main-description">' + escapeHtml(subject.description + " Use the controls on each question to reveal the simple solution or generate a fresh version with new values.") + "</p>" +
          "</div>" +
          '<button id="boostai-main-download" class="boostai-main-download" type="button">↓ Download PDF</button>' +
        "</div>" +
        '<div class="boostai-main-subject-grid">' +
          subjects.map(function (item) {
            return '<button type="button" class="boostai-main-subject' + (item.slug === state.subject ? " active" : "") + '" data-subject="' + item.slug + '">' +
              '<span class="boostai-main-subject-icon">' + escapeHtml(item.icon) + "</span>" +
              "<span>" + escapeHtml(item.name) + "</span>" +
            "</button>";
          }).join("") +
        "</div>" +
        '<div id="boostai-main-status" class="boostai-main-status">Choose a question below and use Solve or Generate similar.</div>' +
        '<div class="boostai-main-question-list">' +
          questions.map(function (question, index) {
            return (
              '<article class="boostai-main-question">' +
                '<div class="boostai-main-question-head">' +
                  '<div class="boostai-main-question-number">' + (index + 1) + "</div>" +
                  '<div class="boostai-main-question-tools">' +
                    '<button type="button" class="boostai-main-action boostai-main-action-outline" data-generate="' + question.id + '">' +
                      "<span>↺</span><span>Generate similar</span>" +
                    "</button>" +
                    '<button type="button" class="boostai-main-action boostai-main-action-solid" data-solve="' + question.id + '">' +
                      "<span>⚡</span><span>" + (question.showSolution ? "Hide solution" : "Solve") + "</span>" +
                    "</button>" +
                  "</div>" +
                "</div>" +
                '<div class="boostai-main-question-content">' +
                  '<p class="boostai-main-question-topic">' + escapeHtml(subject.name + " • " + question.topic) + "</p>" +
                  (question.diagram ? '<div class="boostai-main-diagram">' + renderDiagram(question.diagram) + "</div>" : "") +
                  '<p class="boostai-main-question-prompt">' + escapeHtml(question.prompt) + "</p>" +
                  '<div class="boostai-main-answer-line-wrap"><div class="boostai-main-answer-line"></div></div>' +
                  '<div class="boostai-main-solution' + (question.showSolution ? "" : " hidden") + '">' +
                    "<h3>Simple solution</h3>" +
                    '<ol class="boostai-main-steps">' +
                      question.workedSolution.map(function (step) {
                        return "<li>" + escapeHtml(step) + "</li>";
                      }).join("") +
                    "</ol>" +
                    '<p class="boostai-main-solution-answer">Answer: ' + escapeHtml(question.answer) + "</p>" +
                  "</div>" +
                "</div>" +
                '<div class="boostai-main-footer">(Total for Question ' + (index + 1) + " is " + question.marks + " " + markLabel(question.marks) + ")</div>" +
              "</article>"
            );
          }).join("") +
        "</div>" +
    ...
    Array.prototype.slice.call(mount.querySelectorAll("[data-subject]")).forEach(function (button) {
      button.addEventListener("click", function () {
        setSubject(button.getAttribute("data-subject"));
      });
    });

    Array.prototype.slice.call(mount.querySelectorAll("[data-generate]")).forEach(function (button) {
      button.addEventListener("click", function () {
        regenerateQuestion(button.getAttribute("data-generate"));
      });
    });

    Array.prototype.slice.call(mount.querySelectorAll("[data-solve]")).forEach(function (button) {
      button.addEventListener("click", function () {
        toggleSolution(button.getAttribute("data-solve"));
      });
    });

    var downloadButton = mount.querySelector("#boostai-main-download");
    if (downloadButton) {
